...
where Element: SemanticEquivalenceClassIdentifierProviding {
...
extension SemanticEquivalenceClass {
  ...
  public func contains(element: Element) -> Bool {
    referenceElement.hasIdenticalSemantics(to: element)
      || equivalentElements.containsElementWithSemantics(identicalTo: element)
  }
  ...
  public func hasDistinctSemantics(
    ...
    #if HEAVY_DEBUG
      pedanticAssert(equivalenceClass.isValid)
  ...
  public func isDisjoint(
    ...
    guard !referenceElement.hasIdenticalSemantics(to: equivalenceClass.referenceElement) else {
      return false
    }
    ...
    case (false, true):
      ...
      return !equivalentElements.containsElementWithSemantics(
        equivalentTo: equivalenceClass.referenceElement)
    case (true, false):
      // other has alternates, self doesn't, one last check:
      return !equivalenceClass.equivalentElements.containsElementWithSemantics(
        equivalentTo: referenceElement)
    case (false, false):
      ...
      guard
        !equivalentElements.containsElementWithSemantics(
          identicalTo: equivalenceClass.referenceElement),
        !equivalenceClass.equivalentElements.containsElementWithSemantics(
          identicalTo: referenceElement)
      else {
        return false
      }
  ...
  internal mutating func incorporate(element: Element) {
    ...
    guard !element.shouldBeFavored(over: referenceElement) else {
    ...
    switch equivalentElements.firstIndex(where: { !element.shouldBeFavored(over: $0) }) {
    case .some(let firstDisfavoredIndex):
...
extension SemanticEquivalenceClass {
  ...
  internal var isValid: Bool {
    guard
      !equivalentElements.containsElementWithSemantics(identicalTo: referenceElement),
      equivalentElements.allSatisfy({
        referenceElement.semanticEquivalenceClassIdentifier == $0.semanticEquivalenceClassIdentifier
      }),
...
extension SemanticEquivalenceClass: Equatable where Element: Equatable {}
...
extension SemanticEquivalenceClass: CustomDebugStringConvertible {
  ...
  public var debugDescription: String {
    ...
    return
      "SemanticEquivalenceClass<\(String(reflecting: Element.self))>(referenceElement: \(String(reflecting: referenceElement)), equivalentElements: [ \(elementDebugDescriptions) ])"
  }
