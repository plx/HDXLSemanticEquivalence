...
func semanticEquivalenceTableManualChecks() {
  ...
  let elementsWithExpectedEquivalenceClasses:
    [([PrioritizedString], SemanticEquivalenceClass<PrioritizedString>)] = [
      (
        foos,
  ...
  for removalTarget in [foos, bars, bazs, quuxes].flatMap({ $0 }) {
    ...
    scratchTable.removeEquivalenceClassesSatisfying(predicate: {
      $0.contains(element: removalTarget)
    })
